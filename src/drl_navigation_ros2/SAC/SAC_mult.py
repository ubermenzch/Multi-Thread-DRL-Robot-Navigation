...
class SAC(object):
    ...
    def train(self, replay_buffer, iterations, batch_size):
        ...
        for key, value in self.train_metrics_dict.items():
            if len(value):
                self.writer.add_scalar_mult(key, mean(value), self.step)
    ...
    def add_scalar_mult(self, key, value, step):
    ...
    def update_critic(self, obs, action, reward, next_obs, done, step):
        ...
        self.writer.add_scalar_mult("train_critic/loss", critic_loss, step)
    ...
    def update_actor_and_alpha(self, obs, step):
        ...
        self.writer.add_scalar_mult("train_actor/loss", actor_loss, step)
        self.writer.add_scalar_mult("train_actor/target_entropy", self.target_entropy, step)
        self.writer.add_scalar_mult("train_actor/entropy", -log_prob.mean(), step)
        ...
        if self.learnable_temperature:
            ...
            self.writer.add_scalar_mult("train_alpha/loss", alpha_loss, step)
            self.writer.add_scalar_mult("train_alpha/value", self.alpha, step)
            alpha_loss.backward()
    ...
    def update(self, replay_buffer, step, batch_size):
        ...
        self.writer.add_scalar_mult("train/batch_reward", batch_rewards.mean(), step)
        ...
        if step % 10 == 0:  # 每10步同步一次
            for param in self.critic.parameters():
                dist.all_reduce(param.grad.data, op=dist.ReduceOp.SUM)
                param.grad.data /= num_agents
